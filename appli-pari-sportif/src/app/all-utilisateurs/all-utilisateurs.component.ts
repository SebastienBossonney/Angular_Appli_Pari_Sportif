...
export class AllUtilisateursComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filteredUtilisateur$ = this.identifiant.valueChanges.pipe(
      map((identifiant) =>
        this.utilisateurs.filter((utilisateur) =>
          utilisateur.identifiant.startsWith(identifiant)
        )
      )
    );
  }
  ...
  private getUtilisateur() {
    this.utilisateurService
      ...
      .pipe(
        map(
          (utilisateursObservable) =>
            (this.utilisateurs = utilisateursObservable)
        ),
        takeUntil(this.utilisateurSub)
      )
      .subscribe();
  }
  ...
  add(utilisateur: Utilisateur) {
    this.utilisateurService
      ...
      .subscribe((utilisateur) => this.utilisateurs.push(utilisateur));

    this.swalWithBootstrapButtons.fire(
      '',
      "L' utilisateur " + utilisateur.identifiant + ' a bien été créé.',
      'success'
    );
  }
  ...
  rename(utilisateur: Utilisateur) {
    this.utilisateurService
      .editUtilisateur(utilisateur.id, utilisateur)
      .subscribe(() => {
    ...
    this.swalWithBootstrapButtons.fire(
      '',
      "L' utilisateur " + utilisateur.identifiant + ' a bien été modifié.',
      'success'
    );
  }
  // remove
  remove(utilisateur: Utilisateur) {
    this.utilisateurService.deleteUtilisateur(utilisateur.id).subscribe(() => {
      this.utilisateurs = this.utilisateurs.filter(
        (selectUtilisateur) => selectUtilisateur !== utilisateur
      );
    });
    this.swalWithBootstrapButtons.fire(
      '',
      "L' utilisateur " + utilisateur.identifiant + ' a bien été suprimé.',
      'success'
    );
  }
}
