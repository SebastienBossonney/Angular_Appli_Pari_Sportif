...
export class PariSportMatchComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.matchService.getMatchs(this.sportId).subscribe((data) => {
        this.matchs = data;
  ...
  onChange(event: Event) {
    if (this.matchSelectCH) {
      ...
      this.matchService
        .getCotesByMatchId(this.matchSelectCH.id)
        .subscribe((data) => {
          this.cotesM = data;
  ...
  parier() {
    ...
    if (sommeAParier > user?.montantDisponible) {
      this.swalWithBootstrapButtons.fire(
        '',
        "Vous n'avez pas suffisamment d'argent disponible pour faire ce pari. Veuillez parier moins, racheter de l'argent à votre compte ou bien sûr ne pas parier !",
        'warning'
      );
    }
    //peut parier
      ...
      switch (formValue.radioPari.statut) {
        case 'GAGNANT': {
          ...
          this.equipePari = this.equipe1.nom;
          break;
        }
        case 'PERDANT': {
          console.log(formValue.radioPari.statut);
          this.equipePari = this.equipe2.nom;
          break;
        }
        case 'NUL': {
          console.log(formValue.radioPari.statut);
          this.equipePari = 'Match Nul';
          break;
        }
      }
      ...
      this.swalWithBootstrapButtons.fire(
        '',
        'Bonne Chance! Vous venez de parier ' +
          sommeAParier +
          ' euros à ' +
          this.equipePari +
          ' . Il vous reste maintenant ' +
          user.montantDisponible +
          ' euros. Si la chance est de votre côté, vous gagnerez ' +
          this.pari.montantResultat +
          '!',
        'success'
      );
